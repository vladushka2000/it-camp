...
from dto import auth_dto, http_dto
...
class AuthService(base_service.AbstractService):
    ...
    @classmethod
    async def sign_up_user(cls, user_model: auth_dto.AuthDTO) -> http_dto.HTTPResponseDTO:
        """
        ...
        http_request = http_dto.HTTPRequestDTO(
            url=f"{config.get_auth_host}{enums.AuthEndpoints.SIGN_UP.value}",
            payload={
    ...
    @classmethod
    async def sign_in_user(cls, user_model: auth_dto.AuthDTO) -> http_dto.HTTPResponseDTO:
        """
        Выполнить логику входа пользователя
        :param user_model: данные пользователя
        :return: объект ответа сервера
        """
        ...
        http_request = http_dto.HTTPRequestDTO(
            url=f"{config.get_auth_host}{enums.AuthEndpoints.SIGN_IN.value}",
            payload={
                "name": user_model.name,
                "password": user_model.password
            }
    ...
    @classmethod
    async def refresh_tokens(cls, refresh_token: str) -> http_dto.HTTPResponseDTO:
        """
        Выполнить логику обновления токенов
        ...
        http_request = http_dto.HTTPRequestDTO(
            url=f"{config.get_auth_host}{enums.AuthEndpoints.REFRESH_TOKENS.value}",
            headers={
